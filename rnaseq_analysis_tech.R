...
#meta data ####
#Donors=c("Donor C", "Donor 9")
Time_ps=c("12 days after elec", "two times killing assay")
Vectors=c("AAV6", "AAV8")
...
test_deg_in <- function(Time_p_i,Vector_i) {
    #load data ####
    counts_df <- read.table("/mnt/cgn/data/aavr/RNAseq_batch1_2025_0208/Rawdata/count1.txt", header=T,check.names = F) %>% column_to_rownames("Geneid")
    colnames(counts_df)
    meta_df <- read.table("/home/chengennong/ylp/others/lyj/aavr/rnaseq/metatable.txt", header=T, sep = "\t") %>% dplyr::select(Sample,Group,Donor,Cell_collection_timepoint)
    ...
    meta_df2 <- read.table("/home/chengennong/ylp/others/lyj/aavr/rnaseq/metatable.txt", header=T, sep = "\t") %>% dplyr::select(Sample,Group,Donor,Cell_collection_timepoint) %>% 
      filter(str_detect(Cell_collection_timepoint,Time_p_i) & (str_detect(Group,Vector_i)|str_detect(Group,"Lenti")))
    ...
    colData(dds)$Group <- relevel(colData(dds)$Group, ref = "Lenti")
    dds <- DESeq(dds)   #对原始dds进行normalize
    vst_data <- vst(dds, blind = FALSE)
    pca_assay <- as.data.frame(t(assay(vst_data)))
    #saveRDS(pca_assay, "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/vst_pca_assay_tech.rds")
    resultsNames(dds)
    res <- results(dds)
    summary(res)
    table(res$padj<0.05) 
...
p_dist <-ggplot(deg_result %>% filter(padj < 0.05), aes(x=log2FoldChange, color=Vector, fill=Vector)) +
    ...
    geom_vline(data=mu, aes(xintercept=grp.mean, color=Vector),
               linetype="dashed")+
    scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"), labels = c("AAV6", "AAV8"))+
    scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"), labels = c("AAV6", "AAV8"))+
    ...
    theme_bw()+ scale_color_manual(name="Condition",values=c("#999999", "#E69F00" ), labels = c("AAV6", "AAV8"))
library(ggpubr)
...
ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_dist.pdf",p_dist,height = 2.5, width = 7)



deg_result %>% filter(padj < 0.05 & Comb=="12 days after elec_AAV8")
#Jaccard
...
calculate_jaccard <- function(data) {
  ...
  genes_v2 <- data %>% filter(Vector == "AAV8") %>% pull(Gene)
...
p_num <-ggplot(deg_num_df, aes(x = count, y = Group)) +
  ...
  theme(legend.position="top") + 
  theme_bw()
ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_num.pdf",p_num,height = 3, width = 5.5)
...
ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_volcano.pdf",p_volcano,height = 6, width = 5)
...
wide_data <- deg_result %>% dplyr::select(-Comb) %>%
  ...
  ) %>% mutate(Label = ifelse((Label_AAV8=="Up") & (Label_AAV6=="Up"), "Both up", 
                            ifelse((Label_AAV8=="Down") & (Label_AAV6=="Down"), "Both down","None"))) %>%
    mutate(Label = ifelse((Label_AAV8=="Up" & Label_AAV6=="Down"), "Reverse direction", Label)) %>%
    mutate(Label = ifelse((Label_AAV8=="Down" & Label_AAV6=="Up"), "Reverse direction", Label))


deg_result%>%as.data.frame%>%head
wide_data %>% filter(Label_AAV8=="Up" & Label_AAV6=="Down")
wide_data %>% filter(Label_AAV8=="Down" & Label_AAV6=="Up")
#add label
...
label_deg_resdata_filtered_AAV8 <- wide_data %>% 
  #filter(!str_detect(Row.names,"^TR[ABCD]V")) %>% 
  filter(Label != "None" & Label != "Reverse direction" ) %>% 
  group_by(Label, Time) %>% slice_max(n=5, order_by = abs(log2FoldChange_AAV8)) %>% ungroup 

label_deg_resdata_filtered <- rbind(label_deg_resdata_filtered_AAV6,label_deg_resdata_filtered_AAV8) %>% unique

label_deg_resdata_filtered %>% as.data.frame

#Cor H16
wide_data %>% filter(Time=="12 days after elec") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8))

cor_h16 <- signif(cor(wide_data %>% filter(Time=="12 days after elec") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV6),
                         wide_data %>% filter(Time=="12 days after elec") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV8)),3);cor_h16
cor.test(wide_data %>% filter(Time=="12 days after elec") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV6),
         wide_data %>% filter(Time=="12 days after elec") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV8))$p.value

#H16
p1 <- ggplot(wide_data %>% filter(Time=="12 days after elec"), aes(x=log2FoldChange_AAV6, y=log2FoldChange_AAV8))+
  geom_vline(xintercept = 0, color = "grey60", linewidth = 0.6) +
  geom_hline(yintercept = 0, color = "grey60", linewidth = 0.6) +
  # 不显著的基因为黑色
  geom_point(data = wide_data %>% filter(Time=="12 days after elec") %>% filter(Label=="None"), shape = 21, color = "black", 
             alpha = 0.1, size = 0.1, stroke = 1) 
my_col <- c("Both up" = "#CB181D", "Both down" ="#2927C4", "Reverse direction" = "#C6A58B")
p2 <- p1 + 
  geom_point(data = wide_data  %>% filter(Time=="12 days after elec")%>% filter(Label!="None"), 
             aes(x=log2FoldChange_AAV6, y=log2FoldChange_AAV8, fill = Label, 
                 colour = Label, size = -log10(padj_AAV8), 
                 alpha = -log10(padj_AAV6)),
             shape = 21, stroke = 0.7) + 
  geom_smooth(method = "lm", se = TRUE, col = "orange", alpha=0.5,size=0.7,linetype="dashed")+
  scale_size_continuous(name="-log10(padj_AAV8)",range = c(1.5, 4.5)) +
...
p_FC <- p2 + 
  ggrepel::geom_text_repel(data= label_deg_resdata_filtered %>% filter(Time=="12 days after elec"),
                  aes(x = log2FoldChange_AAV6, y =log2FoldChange_AAV8,label = Row.names),
                  size = 5, color = "black", fontface = 'italic', max.overlaps = 40) +
  #xlim(c(-3, 3)) + 
  #ylim(c(-5, 5)) + 
  xlab("AAV6 vs Lenti (log2FC)") +
  ylab("AAV8 vs Lenti (log2FC)") +
  theme_bw(base_size = 15) 

ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_FC_D12.pdf",p_FC,height = 5, width = 6.5)


#Cor D7
cor_d7 <- signif(cor(wide_data %>% filter(Time=="two times killing assay") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV6),
                         wide_data %>% filter(Time=="two times killing assay") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV8)),3);cor_d7
cor.test(wide_data %>% filter(Time=="two times killing assay") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV6),
         wide_data %>% filter(Time=="two times killing assay") %>% filter(!is.na(log2FoldChange_AAV6) & !is.na(log2FoldChange_AAV8) & Label!="None") %>% pull(log2FoldChange_AAV8))$p.value

#D7
p1 <- ggplot(wide_data %>% filter(Time=="two times killing assay"), aes(x=log2FoldChange_AAV6, y=log2FoldChange_AAV8))+
  geom_vline(xintercept = 0, color = "grey60", linewidth = 0.6) +
  geom_hline(yintercept = 0, color = "grey60", linewidth = 0.6) +
  # 不显著的基因为黑色
  geom_point(data = wide_data %>% filter(Time=="two times killing assay") %>% filter(Label=="None"), shape = 21, color = "black", 
             alpha = 0.1, size = 0.2, stroke = 1) 
my_col <- c("Both up" = "#CB181D", "Both down" ="#2927C4", "Reverse direction" = "#C6A58B")
p2 <- p1 + 
  geom_point(data = wide_data  %>% filter(Time=="two times killing assay")%>% filter(Label!="None"), 
             aes(x=log2FoldChange_AAV6, y=log2FoldChange_AAV8, fill = Label, 
                 colour = Label, size = -log10(padj_AAV8), 
                 alpha = -log10(padj_AAV6)),
             shape = 21, stroke = 0.7) + 
  ...
  scale_fill_manual(values = my_col ) +
  scale_color_manual(values = my_col )+labs(title=paste0(paste("The correlation of DEGs\nPearson r = ",cor_d7),
                                                               "\np-value = 1.5e-14 (DEGs only)"))

p_FC <- p2 + 
  ggrepel::geom_text_repel(data= label_deg_resdata_filtered %>% filter(Time=="two times killing assay"),
                  aes(x = log2FoldChange_AAV6, y =log2FoldChange_AAV8,label = Row.names),
                  size = 5, color = "black", fontface = 'italic', max.overlaps = 40) +
  #xlim(c(-3, 3)) + 
  #ylim(c(-5, 5)) + 
...
ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_FC_TT.pdf",p_FC,height = 5, width = 6.5)

#heatmap ####
counts_df0 <- read.table("/mnt/cgn/data/aavr/RNAseq_batch1_2025_0208/Rawdata/count1.txt", header=T) 
length_df0 <- read.table("/mnt/cgn/data/aavr/RNAseq_batch1_2025_0208/Rawdata/counts.txt", header=T) %>% dplyr::select(1,6)
...
cols <- rev(colorRampPalette(RColorBrewer::brewer.pal(11, "PiYG"))(50))

mat  <- (log(tpm_value+1))[c(#as.data.frame(label_deg_resdata_filtered) %>% arrange(log2FoldChange_AAV8)%>% pull(Row.names),
...
#
#ann_colors <- list(
#  Group = c(`Lenti` = "black", `AAVR OE + AAV6` = '#999999', `AAVR OE + AAV8` = '#E69F00'),  
#  Donor = c(`Donor 9` = "#86A8E7", `Donor C` = "#D16BA5"),
#  Cell_collection_timepoint = c(`12 days after elec` = "#40AA40", `two times killing assay` = "#AAAA40")           
#)
...
#         show_colnames = F,
#           height = 7, width = 8,
#           filename = paste0("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_",name,"_gene.pdf"))
#}
...
dim(mat)
#as.data.frame(label_deg_resdata_filtered) %>% arrange(log2FoldChange_AAV8)%>% pull(Row.names)
tpm_value["MT-ATP8",]
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 7, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_all_gene.pdf")
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 2.3, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_naive_gene.pdf")
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 4, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_cyt_gene.pdf")
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 2.9, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_ex_gene.pdf")
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 3.3, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_la_gene.pdf")
...
pheatmap::pheatmap(t(result), annotation_col = anno1,
         ...
         show_colnames = F,
         height = 6.0, width = 6,
         filename = "/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot_tech/p_heatmap_act_gene.pdf")
