...
library(dplyr)
library(FactoMineR)
meta_df <- read.table("/home/chengennong/ylp/others/lyj/aavr/rnaseq/metatable.txt", header=T, sep = "\t") %>% select(Sample,Group,Donor,Cell_collection_timepoint)
meta_df

pca_assay <- readRDS("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot/vst_pca_assay.rds")
...
p <- ggplot(pca_df, aes(x=Dim.1, y=Dim.2,shape=paste(Donor),fill=Donor, colour=Group,alpha=Cell_collection_timepoint)) + 
  ...
  scale_alpha_manual(name="Time point",values = c(0.4,1), labels = c("12 days after elec", "two times killing assay"))+
    scale_color_manual(name="Condition",values=c("black","#999999", "#E69F00" ), labels = c("Lenti", "AAVR OE + AAV6", "AAVR OE + AAV8"))+
    labs(shape = "Donor", x= paste0("PC1 (",round(pca_var[1],2),"%)"), y= paste0("PC2 (",round(pca_var[2],2),"%)"))
ggsave("/home/chengennong/ylp/others/lyj/aavr/rnaseq_yecode/plot/pca_vst.pdf", width = 5.8, height = 4)
